...
### Task 8 — amendement après revue et vérification visuelle (`13966f4` + correctif `6dbd07e`)
Vérifié par le coordinateur sur 13 offres fictives semées (`pnpm --filter @jobtrack/api jobs:seed`) : liste, sous-titre, reprise des préférences dans l'URL, filtres contrat + salaire minimum + tri salaire (ordre des bornes hautes correct), onglet « Nouvelles », télétravail hybride + confirmé, sauvegarde (204, `aria-pressed`), mobile (barre « Filtres (n) », pas de débordement), sombre. Critique corrigé : « Actualiser » était inerte (la clé de cache ignore `refresh`, donc aucune requête ne partait) → drapeau consommé par la `queryFn` via une référence + `refetch()` explicite, test dédié ; « Actualiser » aussi sur `degraded` et `ok`. Aussi : course entre la reprise des préférences et une saisie de l'utilisateur (relecture de l'URL avant d'appliquer) ; salaire minimum anti-rebond 400 ms avec `replace` ; `useId` sur le champ rendu deux fois (rangée + `Sheet`) ; combobox des communes avec `aria-activedescendant`, `aria-live`, retour du focus, saisie possible au maximum avec `aria-describedby` ; déclencheurs d'info-bulle focusables (télétravail déduit, onglets désactivés `role="button" aria-disabled`) ; barre de recherche collante sur mobile ; pagination sans points de suspension pour un trou d'une page ; « Réinitialiser les filtres » seulement si un filtre est actif ; `sync.message` affiché ; `memo(JobCard)`. web 161 → 187.

### Task 6 — amendement après revue sécurité (`b66a19f` + correctif `6ad43a2` + `fef9598`)
Auth/CSRF globaux, `saved` avant `:id`, clés inconnues éliminées, isolation des favoris, DTO sans champs internes, journaux propres : confirmés. Critique corrigé : la re-vérification d'expiration sur `GET /jobs/:id` n'avançait pas `lastSeenAt` en cas d'erreur ou d'offre disparue → un seul id pouvait saturer le quota France Travail ; désormais `expiredAt`/`lastSeenAt` testés avant tout appel et marqueur Redis `jobs:detail-check:{id}` (1 h) posé **avant** l'appel, quel qu'en soit le résultat. Importants : budget par utilisateur sur les **synchronisations implicites** (30 / 10 min via `allowSync`, refus servi en `cached` avec message, sans 429) — la spec §8 supposait à tort que le cache bornait des requêtes distinctes ; métacaractères LIKE échappés (`q=%` renvoyait tout) ; hygiène e2e : préfixe `E2E-` pour le connecteur factice, nettoyage limité aux hachages créés et aux clés Redis de la suite (jamais `jobs:ft:token`, `jobs:communes:loadedAt` ni `ratelimit:*` en bloc) — les 13 offres semées survivent à une exécution. Mineurs : `Object.hasOwn` sur les tables de clés (pipe et contrat), `rateLimitKey` partagé, tri salaire avec `id` en départage + index `(salaryMaxAnnual, publishedAt)` (migration `20260916215515`), `page ≤ 500`, `q` des communes borné à 120 et clé vide → `[]`, garde de type dans le connecteur factice, e2e sur `lieu` répété, clés inconnues et fuite de DTO. Après coup (`fef9598`) : les tests des communes dépendaient de `ensureLoaded()`, gardé par la clé Redis de 30 jours posée par une exécution antérieure alors que les lignes avaient été nettoyées → référentiel vide ; la suite insère désormais ses communes de test directement. api e2e 77 → 115.

### Task 9 — amendement après revue et vérification visuelle (`f9bf90c` + correctif `f88e47b`)
Vérifié par le coordinateur sur une offre semée : en-tête, badges, description, compétences exigées/souhaitées, formations et langues, entreprise, conditions, « Disponible sur », liens externes tous en `noopener noreferrer`, sauvegarde depuis le détail, page Favoris avec compteur puis retrait → état vide. Critique corrigé : sous 640 px la barre d'actions fixe était recouverte par la navigation basse (même `z-index`, même `bottom-0`) → repositionnée au-dessus (`bottom-[calc(4rem+env(safe-area-inset-bottom))]`). Aussi : bascule « Voir plus » avec `aria-expanded`/`aria-controls` ; 404 dédié sans « Réessayer » ; info-bulle du télétravail déduit focusable ; un seul contrôle de sauvegarde (icône + texte dans le même bouton) ; **libellés bruts** affichés quand le parsing échoue (salaire, expérience, « Débutant accepté ») ; `isHttpUrl` centralisé ; CTA sur la première source `http(s)` ; noms accessibles des liens de sources ; états des favoris mutuellement exclusifs. Reporté : « déplacements » (spec §2) sans champ de DTO ; titres de sections en `div` (choix global de `Card`). web 187 → 195.
