...
export function createEslintConfig(tsconfigRootDir) {
  return tseslint.config(
    ...
    ...tseslint.configs.recommendedTypeChecked,
    {
      languageOptions: {
        parserOptions: {
          // `prisma/*.ts` couvre le script de seed, hors du `include` de src.
          // `**` est refusé par typescript-eslint : n'utilise que des motifs simples.
          projectService: { allowDefaultProject: ['*.js', '*.config.*', 'prisma/*.ts'] },
          tsconfigRootDir,
        },
      },
      rules: {
...
/** Configuration appliquée à `packages/config` lui-même. */
export default createEslintConfig(import.meta.dirname);
