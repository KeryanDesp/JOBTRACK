 ...
 * l'évidence), comparé aux modes acceptés par le profil. Un profil sans
 * préférence de télétravail est considéré compatible avec tout mode.
 * `unknown` si ni l'analyse ni l'offre n'indiquent de mode.
 */
export function scoreRemote(profile: ProfileInputs, job: JobInputs, requirements: JobRequirements, _now: Date): MatchFactorDto {
  ...
  const compatible = profile.remoteModes.length === 0 || profile.remoteModes.includes(effective);
