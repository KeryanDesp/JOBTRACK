...
/** Complète des listes du profil quand une extraction de CV en a deviné (accueil), sans champ déjà rempli. */
export interface PreferencesExtraDefaults {
  desiredRoles?: readonly string[];
  locations?: readonly string[];
}

function toFormValues(preferences: PreferencesDto, extraDefaults?: PreferencesExtraDefaults): PreferencesFormValues {
  const desiredRoles = preferences.desiredRoles.length > 0 ? preferences.desiredRoles : (extraDefaults?.desiredRoles ?? []);
  const locations = preferences.locations.length > 0 ? preferences.locations : (extraDefaults?.locations ?? []);
  return {
    desiredRoles: joinTags(desiredRoles),
    desiredCategories: joinTags(preferences.desiredCategories),
    salaryMin: preferences.salaryMin ?? '',
    salaryMax: preferences.salaryMax ?? '',
    locations: joinTags(locations),
...
export interface PreferencesFormProps {
  /** Pré-remplissage des postes/lieux recherchés depuis une extraction de CV, si le profil n'en a pas déjà. */
  extraDefaults?: PreferencesExtraDefaults;
  /** Texte du bouton de soumission (« Enregistrer » par défaut, « Continuer » à l'accueil). */
  submitLabel?: string;
  /** Le bouton reste actif même sans modification — utile à l'accueil où « Continuer » doit toujours avancer. */
  requireDirty?: boolean;
  /** Appelé après un enregistrement réussi, avec les préférences à jour. */
  onSaved?: (updated: PreferencesDto) => void;
}

/**
 * Formulaire des préférences de recherche, réutilisé tel quel par
 * `PreferencesCard` (profil) et par l'étape « Préférences » de l'accueil.
 * Porte son propre chargement/erreur/soumission ; l'appelant ne fournit que
 * `<Card>` (ou rien, à l'accueil) autour.
 */
export function PreferencesForm({ extraDefaults, submitLabel = 'Enregistrer', requireDirty = true, onSaved }: PreferencesFormProps) {
  ...
  const disabled = (requireDirty && !form.formState.isDirty) || mutation.isPending;
